...
class TestDigitalTyphoonDataset(TestCase):
    ...
    def test_transform_func_transforms(self):
        ...
        for i in range(len(first_values)):
            ...
            if read_in_image[-1][-i-1] != last_values[-i-1]:
                self.fail(f'Value produced was {read_in_image[-1][-i-1]}. Should be {last_values[-i-1]}')
        test_dataset = self.create_test_dataset(transform_func=lambda img: np.ones(img.shape),spectrum='Infrared')
    ...
    def test_random_split_by_image_random_produces_nonidentical_indices(self):
        ...
        for i in range(min(len(bucket1_1), len(bucket1_2))):
            if bucket1_1.indices[i] != bucket1_2.indices[i]:
                all_same = False
                break
        self.assertFalse(all_same, "Random split produced identical indices in bucket1")
        ...
        for i in range(min(len(bucket1_1), len(bucket1_2))):
            if bucket1_1.indices[i] != bucket1_2.indices[i]:
                all_same = False
                break
        self.assertFalse(all_same, "Random split produced identical indices in bucket1 (three-way split)")

        for i in range(min(len(bucket2_1), len(bucket2_2))):
            if bucket2_1.indices[i] != bucket2_2.indices[i]:
                all_same = False
                break
        self.assertFalse(all_same, "Random split produced identical indices in bucket2")

        for i in range(min(len(bucket3_1), len(bucket3_2))):
            if bucket3_1.indices[i] != bucket3_2.indices[i]:
                all_same = False
                break
        self.assertFalse(all_same, "Random split produced identical indices in bucket3")
    ...
    def test_assign_all_images_dataset_idx(self):
        ...
        counts_should_be = {'200801': 157, '200802': 175, '197918': 50, '201323': 193, '202222': 193}
    ...
    def test_get_image_from_idx_as_numpy(self):
        test_dataset = DigitalTyphoonDataset(IMAGE_DIR, METADATA_DIR,
                                             METATADA_JSON, 'grade', spectrum='Infrared', verbose=False)
    ...
    def test_get_nonempty_seasons_and_sequences(self):
        ...
        test_dataset = DigitalTyphoonDataset(IMAGE_DIR, METADATA_DIR,
                                             METATADA_JSON, 'grade', filter_func=filter_func, verbose=False)
